...
function matchError(error, retryArray) {
    // Clean up the error name. Some user code sets .name, others rely on .message
    ...
    for (let i = 0; i < retryArray.length; i++) {
        ...
        for (const e of retryBlock.ErrorEquals) {
            // For a custom string or known "States.*", or "States.ALL"
