...
const { IOCtrl } = require('./io');
const { findChoice } = require('./rules');
...
function Handlers(context) {
    ...
    return {
        async Task(state, io) {
            const fn = typeof state.Resource === 'function' ? state.Resource
                     : state.Resource.then                  ? await state.Resource
                     : /* default */                          await context.resources[state.Resource];
