...
const stateHandlers = {
    ...
    async Task(context, input) {
        ...
        try {
            ...
            const promises = [context.handlers[state.Resource](input, signal)];
...
function initializeContext(context, machineDef, input) {
    const signal = context?.signal; // Preserve the signal before cloning
    context = deepClone(context ?? {});
    if (signal) {
        context.signal = signal; // Restore the signal after cloning
    }
