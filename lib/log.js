...
export const print = (depth, colors, labels, ...args) =>
    printLog(depth, colors, labels, ...args);

export const log = (depth, colors, labels, ...args) =>
    console.log(print(depth, colors, labels, ...args));

export const logStateInfo = (context, event, ...args) =>
    log(context.depth,
        [chalk.yellow, chalk.dim, chalk.blue],
        [context.stateKey, context.state.Type, event],
        ...args);

export const logStateSucceed = (context, event, ...args) =>
    log(context.depth,
        [chalk.yellow, chalk.dim, chalk.green],
        [context.stateKey, context.state.Type, event],
        ...args);

export const logStateFail = (context, event, ...args) =>
    log(context.depth,
        [chalk.yellow, chalk.dim, chalk.red],
        [context.stateKey, context.state.Type, event],
        ...args);

export const logMachineInfo = (context, key, event, ...args) =>
    log(context.depth,
        [chalk.dim, chalk.dim, chalk.blue],
        [key, 'Machine', event],
        ...args);

export const logMachineSucceed = (context, key, event, ...args) =>
    log(context.depth,
        [chalk.dim, chalk.dim, chalk.green],
        [key, 'Machine', event],
        ...args);

export const logMachineFail = (context, key, event, ...args) =>
    log(context.depth,
...
export function DefaultLogger(context) {
    return (event, ...args) => {
        if (event == 'StateInfo') {
            ...
            return logMachineInfo(context, '+', 'MachineStarted', ...args);
        } else if (event == 'MachineSucceed') {
            return logMachineSucceed(context, '-', 'MachineSucceeded', ...args);
        } else if (event == 'MachineFail') {
            return logMachineFail(context, '-', 'MachineFailed', ...args);
        }
    };
}
