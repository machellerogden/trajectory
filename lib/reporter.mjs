...
const Log = type => event => {
    ...
    const exitStatusMark = compact                     ? ''
                         : closed && type === 'stdout' ? chalk.green.bold('✔')
                         : closed && type === 'stderr' ? chalk.red.bold('✖')
                         : '';
    let stdio;

    const prefixName = fixedWidth(gutterWidth, name);

    const prefix = compact ? '' : `${pad}${chalk[color].bold(prefixName)} ${chalk.grey('│')} `;
    let output = prefix;

    if ([ 'stdout', 'stderr' ].includes(type)) {
        if (data == null) {
            if (!compact) output += `${pad}${exitStatusMark}${EOL}`;
        } else {
...
const logHandlers = Object.keys(logTypes).reduce((r, t) => (r[t] = Log(t), r), {});
